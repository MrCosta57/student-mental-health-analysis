...
# Reduce the number of categories for the symptoms variables (except "internet_addiction")
...
for (col_name in general_cols) {
  plot_counts(df, col_name, "suicide", "True", var_name_to_label(col_name, ""), "Count [suicide=TRUE]", default_colors[1])
}
```

```{r fig.width=4, fig.height=2}
for (col_name in general_cols) {
  plot_counts(df, col_name, "suicide", "False", var_name_to_label(col_name, ""), "Count [suicide=FALSE]", default_colors[1])
}
```

## Symtoms information
```{r fig.width=4, fig.height=2}
for (col_name in symptoms_cols) {
  plot_counts(df, col_name, "suicide", "True", var_name_to_label(col_name, ""), "Count [suicide=TRUE]", default_colors[2])
}
```

```{r fig.width=4, fig.height=2}
for (col_name in symptoms_cols) {
  plot_counts(df, col_name, "suicide", "False", var_name_to_label(col_name, ""), "Count [suicide=FALSE]", default_colors[2])
}
```

## Interaction terms
```{r}
plot_interaction_exploration(df_original, "suicide", "psychiatric_symptoms", df, "gender", default_colors, 0.5, var_name_to_label("psychiatric_symptoms"), var_name_to_label("suicide"))
```
...
mod_step <- step(mod_zero, direction = "both", scope=formula(mod_full), trace=0) 
```

```{r}
residualPlots(mod_step)
qqPlot(residuals(mod_step))
```

## Lasso
```{r}
mod_lasso <- cv.glmnet(suicide ~ ., data = df_train, family = "binomial", alpha = 1)
coef_lasso <- coef(mod_lasso, s=mod_lasso$lambda.1se)
coeff_lasso_idx <- which(coef_lasso != 0, arr.ind = TRUE)
print(cbind(coeff_lasso_idx, coef_lasso[coeff_lasso_idx])[,3])
