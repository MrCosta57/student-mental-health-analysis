...
```{r}
for (i in extract_predictors(mod_step$formula, general_cols)){
  plot(effect(i, mod_step), ylab = "Probability of Suicide", rescale.axis = FALSE)
}
```

```{r}
for (i in extract_predictors(mod_step$formula, symptoms_cols)){
  plot(effect(i, mod_step), ylab = "Probability of Suicide", rescale.axis = FALSE)
}
```
...
coef_lasso1se <- coef(mod_lasso, s = mod_lasso$lambda.1se)[, 1]
coeff_lasso1se_idx <- which(coef_lasso1se != 0)

print(cbind(coeff_lasso1se_idx, coef_lasso1se[coeff_lasso1se_idx])[, 2])
```
...
df_train <- df[idx, ]
df_test <- df[-idx, ]
#full_gam_formula <- as.formula(paste("I(suicide>threshold_suicide)", "~", paste(general_cols, collapse = " + "), " + ", paste("s(", symptoms_cols, ")", collapse = "+")))

#mod_gam <- gam(full_gam_formula, family = binomial, data = df_train)
#summary(mod_gam)
