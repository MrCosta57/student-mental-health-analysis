...
for (i in extract_predictors_in_vec(mod_step_aic$formula, general_cols)) {
  plot(effect(i, mod_step_aic), ylab = "Probability of Suicide", rescale.axis = FALSE)
}
```
...
```{r}
mod_lasso <- cv.glmnet(suicide ~ ., data = df_train, family = "binomial", alpha = 1, nfolds = 10)
beta_1se <- coef(mod_lasso, s = mod_lasso$lambda.1se)
beta_1se_idx <- which(beta_1se[, 1] != 0)
print(format(cbind(beta_1se_idx, beta_1se[beta_1se_idx])[, 2], scientific = FALSE))
...
```{r message=FALSE, warning=FALSE}
probs_gam <- predict(mod_gam, newdata = df_test, type = "response")
roc_gam <- roc(df_test$suicide ~ probs_gam, plot = TRUE, print.auc = TRUE)
