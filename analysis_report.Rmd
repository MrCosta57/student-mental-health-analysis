...
From the above table, it can be seen that all the columns have the expected format now. Also can be observed that there are no missing values but some duplicate rows are present. Duplicates in this case probably refer to students that have the same profile conditions and that have replayed in the same way to the survey for building the dataset. It's chosen to keep these rows to maintain the original dataset semantics.
...
However, this information is not fully considered because numeric variables have all positive variables and they are almost all in the same range.
...
- The model fitted on all the predictors reach almost a score similar to the previous one but maybe the overfits a bit on the data, considering this results
