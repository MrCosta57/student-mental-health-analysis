...
default_colors <- palette.colors()[2:length(palette.colors())]
requirements <- c("summarytools", "dplyr", "car", "effects", "stringr", "ggplot2", 
                  "scales", "pROC", "glmnetUtils", "mgcv", "MASS", "comprehenr")
...
for (col_name in general_cols) {
  plot_counts(df, col_name, "suicide", "True", var_name_to_label(col_name, ""), "Count", default_colors[1])
}
```
...
for (col_name in symptoms_cols) {
  plot_counts(df, col_name, "suicide", "True", var_name_to_label(col_name, ""), "Count", default_colors[2])
}
```
...
mod_step <- step(mod_zero, direction = "both", scope=formula(mod_full)) 
```
...
coords(roc_stepwise, x = "best", ret = c("threshold", "specificity", "sensitivity", "accuracy","tn","tp","fn","fp"))
...
coords(roc_lasso, x = "best", ret = c("threshold", "specificity", "sensitivity", "accuracy","tn","tp","fn","fp"))
```
